...
def sort_by_cohort(filename):
    ...
    tuple_list = [(fall_15, "Fall 2015"), (winter_16, "Winter 2016"), (spring_16, "Spring 2016"), (summer_16, "Summer 2016"), (ghosts, "G")]
    ...
    for line in lst_houses:
        ...
        for item in tuple_list:
            if token[4] == item[1]:
                item[0].append(token[0] + " " + token[1])

    all_students = [indiv_tuple[0] for indiv_tuple in tuple_list]
...
def all_students_tuple_list(filename):
    ...
    for line in lst_houses:
        ...
        #combine name tokens to make full name
        full_name = token[0] + " " + token[1]

        #replace first and last name strings with full name strings
        # token[0] = full_name
        # del token[1]

        token[0:2] = [full_name]

        #convert token list to tuples
        ind_tuple = tuple(token)

        if ind_tuple[3] != "I" and ind_tuple[3] != "G":
        #append student list with each tuple
            student_list.append(ind_tuple)
...
def find_cohort_by_student_name(student_list):
    ...
    # Code goes here
    user_name = raw_input("Who are you looking for? ")
...
def find_house_members_by_student_name(student_list):
    ...
    for item in student_list:
        if item[0] == student_name:
            house_name = item[1]
            cohort_name = item[3]
    print "{} was in {} in the {} cohort.\nThe following students are also in their house:".format(student_name, house_name, cohort_name)
    for student in student_list:
        if student[1] == house_name and student[3] == cohort_name and student[0] != student_name:
            print student[0]
    return
